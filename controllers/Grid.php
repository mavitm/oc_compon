...
class Grid extends Controller
{
    ...
    public function __construct()
    {
        ...
        BackendMenu::setContext('Mavitm.Compon', 'compon', 'grid_menu');
    }
        ...
        if(!empty($this->params[0])){
            if( $this->params[0] == "subcreate" )
            {
                if(!empty($this->params[1])){
                    ...
                    $this->pageTitle = $parent->title.' - '.e(trans('mavitm.compon::lang.grid.newItem'));
                }
            }
        }
    ...
    public function update()
    {
        ...
        if( !empty($this->params[1]) )
        {
            if(!empty($this->params[1])){
                $parent = Mtmdata::where("id",$this->params[1])->first();
                $this->pageTitle = $parent->title.' - '.e(trans('mavitm.compon::lang.grid.edit'));
            }
        }
